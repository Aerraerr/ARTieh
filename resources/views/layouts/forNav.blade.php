...
<nav style="max-width: 100%; height: 60px;" class="flex justify-between items-center px-10 py-6 shadow-sm bg-transparent">
    ...
    <div class="hidden md:flex space-x-4">
            ...
            <div class="flex items-center space-x-2"> <!-- Flex container to keep elements close -->
                <span id="navusername" class="text-gray-800 text-sm font-semibold"> 
...
<div  id="mobileMenu" class="hidden fixed inset-0 bg-white flex flex-col items-center justify-start space-y-5 shadow-md z-40 pt-10" style="font-family: 'Rubik', sans-serif;">
    ...
    <hr class="w-3/4 border-t border-[#6e4d41] my-4 mx-auto">

    <div class="flex flex-col items-center space-y-2 md:hidden">
    @auth
        <!-- Display user email or name -->
        <span id="navusername" class=" text-[#6e4d41] text-sm"> {{ explode('@', Auth::user()->email)[0] }} </span>
        <!-- View Profile Button -->
        <div class="flex justify-center mb-3">
            <a href="{{ route('forViewProfile') }}" 
                class="w-full text-center px-4 py-2 border border-[#6e4d41] text-[#6e4d41] rounded-lg hover:bg-[#6e4d41] hover:text-white transition">
                View Profile
            </a>
        </div>
        <!-- Logout Button -->
        <form style="margin-top:-10px;" action="{{ route('logout') }}" method="POST" class="w-full h-10 flex items-center justify-center bg-[#6e4d41] text-white rounded-lg hover:bg-[#A99476] hover:text-gray-100 transition">
            @csrf
